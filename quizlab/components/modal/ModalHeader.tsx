...
const ModalHeader = ({
  ...
  return (
    <View className="w-full flex flex-row h-[56px] items-center p-6 border-b border-gray-5">
      {variant === "back" && (
        <View className="flex flex-row w-full justify-start items-center">
          <TouchableOpacity onPress={onPressBack} activeOpacity={0.8}>
            <Feather name="chevron-left" size={16} color={Gray.black} />
          </TouchableOpacity>
      ...
      {variant === "close" && (
        <View className="flex flex-row w-full justify-between items-center">
          ...
          <TouchableOpacity onPress={onPressClose} activeOpacity={0.8}>
            <Feather name="x" size={16} color={Gray.black} />
          </TouchableOpacity>
        </View>
      )}
    </View>
